...
export default function FetchDataComponent(props: any) {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            setIsLoading(true);
            try {
                const response = await fetch("/api/fetchPosts", { cache: 'no-store' }).then(result => result.json());
                            ...
                            posts?.map((post) => (
                                <div className="p-4 my-2 rounded-md border-b leading-8" key={post._id}>
                                    ...
                                    <div className="flex gap-4 mt-4 justify-end">
                                        ...
                                        <button onClick={() => deleteHandler(post._id)} className="bg-red-500 px-4 py-2 rounded-md uppercase text-sm font-bold tracking-widest text-white">
                                            Delete
                                        </button>
                                    </div>
                                </div>
